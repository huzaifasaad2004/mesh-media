...
import { DollarSign, AlertCircle, TrendingDown, TrendingUp, Trophy } from 'lucide-react'
    ...
    <div className="mb-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
        {[
          ...
          { label: 'Net Profit (period − monthly payroll)', value: report?.netProfit ?? 0, icon: TrendingUp, color: (report?.netProfit ?? 0) >= 0 ? 'text-green-700' : 'text-red-600', bg: (report?.netProfit ?? 0) >= 0 ? 'bg-green-50' : 'bg-red-50' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="card p-4">
          <p className="text-sm font-semibold text-ink mb-3">Revenue trend · last 12 months</p>
          {report && report.trend.length > 0 ? (
            ...
            <p className="text-sm text-taupe-500 py-6 text-center">No paid invoices yet</p>
          )}
        </div>
        ...
        <div className="card p-4">
          ...
          {report && report.topClients.length > 0 ? (
            ...
            <p className="text-sm text-taupe-500 py-6 text-center">No paid revenue in this period</p>
          )}
        </div>
      </div>
    </div>
