...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  let query = serviceRole().from('chat_messages')
    .select('*, sender:profiles!chat_messages_sender_id_fkey(id, full_name, email, avatar_url), reply:chat_messages!chat_messages_reply_to_id_fkey(id, body, sender:profiles!chat_messages_sender_id_fkey(full_name)), reactions:chat_reactions(emoji, user_id)')
    .eq('channel_id', params.id).order('created_at', { ascending: false }).limit(60)
  if (before) query = query.lt('created_at', before)
  const { data, error } = await query
  if (error) return NextResponse.json({ error: error.message }, { status: 400 })
  const messages = await attachSignedUrls((data ?? []).reverse())
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await db.from('chat_messages').insert({ channel_id: params.id, sender_id: auth.user.id, body: text, reply_to_id: body.reply_to_id || null }).select('*').single()
  if (error) return NextResponse.json({ error: error.message }, { status: 400 })
  await Promise.all([
