...
export async function GET(req: NextRequest) {
  ...
  let invoiceQuery = db.from('invoices').select('id, total, status, paid_date, client_id, client:clients(company_name)').eq('status', 'paid')
  ...
  const [{ data: paidInvoices }, { data: expenses }, { data: outstandingInvoices }, { data: salaries }, { data: last12moInvoices }] =
    await Promise.all([
      ...
      db.from('invoices').select('total, paid_date').eq('status', 'paid').gte('paid_date', new Date(new Date().setMonth(new Date().getMonth() - 11)).toISOString().split('T')[0]),
    ])
  ...
  // Top clients by paid revenue in this period
