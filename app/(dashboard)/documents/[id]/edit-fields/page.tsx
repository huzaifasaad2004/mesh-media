...
const FIELD_COLOR: Record<Party, string> = { agency: '#6E1318', client: '#4A5A6E' }
...
export default function EditFieldsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (docRes.ok) setDoc(docData)
      if (fieldsRes.ok && Array.isArray(fieldsData)) {
        setFields(fieldsData.map((f: any) => ({
          id: f.id, page_number: f.page_number, field_type: f.field_type, assigned_party: f.assigned_party,
  ...
  const onMouseUp = () => {
    if (!dragStart.current || !dragBox) { dragStart.current = null; return }
  ...
  const save = async () => {
    ...
    const res = await fetch(`/api/documents/${id}/fields`, {
      ...
      body: JSON.stringify({ fields: fields.map(({ page_number, field_type, assigned_party, x, y, width, height }) => ({ page_number, field_type, assigned_party, x, y, width, height })) }),
    })
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-4 mb-4 items-center">
        ...
        <div className="flex gap-1.5 text-xs">
          {(['agency', 'client'] as Party[]).map((p) => (
            <button key={p} onClick={() => setParty(p)} className={`px-3 py-1.5 rounded-md capitalize ${party === p ? 'text-white' : 'bg-paper-100 text-umber-700'}`} style={party === p ? { background: FIELD_COLOR[p] } : {}}>{p}</button>
          ))}
        </div>
        {numPages > 1 && (
      ...
      <div className="flex gap-6 flex-wrap items-start">
            ...
            {pageFields.map((f) => (
              <div
                ...
                style={{
                  ...
                  background: `${FIELD_COLOR[f.assigned_party]}CC`,
                }}
              >
                {f.field_type}
        ...
        <div className="card p-4 w-full max-w-xs">
          ...
          {fields.length === 0 ? (
            <p className="text-xs text-taupe-500">Click and drag on the document to place a field, or just click for a default-sized box.</p>
          ) : (
            <ul className="space-y-1.5 text-xs">
              {fields.map((f) => (
                <li key={f.id} className="flex items-center justify-between gap-2 px-2 py-1.5 rounded bg-paper-50">
                  <span className="capitalize">{f.field_type} · {f.assigned_party} · p{f.page_number}</span>
