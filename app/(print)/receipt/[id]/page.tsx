...
export default function ReceiptPrintPage() {
  ...
  useEffect(() => {
    const url = token ? `/api/receipt/${id}?token=${token}` : `/api/receipt/${id}`
    fetch(url)
    ...
    <>
      <div className="no-print" style={{
        ...
        <span style={{ color: 'white', fontWeight: 600, fontSize: 13, flex: 1 }}>
          Payment Receipt · {fmtDate(payment.payment_date)}
        </span>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 22 }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontFamily: 'Cormorant, Georgia, serif', fontWeight: 700, fontSize: 34, color: BRAND, lineHeight: 1 }}>RECEIPT</div>
                <div style={{ fontSize: 12, color: '#555', marginTop: 4 }}>{fmtDate(payment.payment_date)}</div>
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontWeight: 700, fontSize: 15, color: BRAND }}>{payment.contractor?.name ?? 'Contractor'}</div>
              {payment.contractor?.email && <div style={{ fontSize: 11.5, color: '#555' }}>{payment.contractor.email}</div>}
            </div>
              ...
              <tbody>
                {payment.project?.name && (
                  <tr style={{ borderBottom: '1px solid #f0ebe6' }}>
                    <td style={{ padding: '8px 0', color: '#888', fontSize: 12 }}>Project</td>
                    <td style={{ padding: '8px 0', textAlign: 'right', fontWeight: 600, fontSize: 12 }}>{payment.project.name}</td>
                  </tr>
                )}
                <tr style={{ borderBottom: '1px solid #f0ebe6' }}>
                ...
                {payment.description && (
                  <tr style={{ borderBottom: '1px solid #f0ebe6' }}>
                    <td style={{ padding: '8px 0', color: '#888', fontSize: 12 }}>Description</td>
                    <td style={{ padding: '8px 0', textAlign: 'right', fontSize: 12 }}>{payment.description}</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            <div style={{
              ...
              <span style={{ color: CREAM, fontSize: 13, fontWeight: 600 }}>Amount Paid</span>
              <span style={{ color: 'white', fontSize: 24, fontWeight: 700, fontFamily: 'Cormorant, Georgia, serif' }}>
                {payment.currency ?? 'AED'} {Number(payment.amount).toLocaleString('en-US', { minimumFractionDigits: 2 })}
              </span>
            </div>
