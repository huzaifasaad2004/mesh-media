...
type Props = { clientName: string; projectName?: string; start: string; end: string; timezone: string; commentary?: string; data: any; baseUrl: string }
...
function CampaignReport({ clientName, projectName, start, end, timezone, commentary, data, baseUrl }: Props) {
  ...
  return <Document title={`${clientName} Campaign Performance Report`} author={COMPANY.name}>
    <Page size="A4" style={st.page}>
      ...
      <View style={{ backgroundColor: PALE, padding: 12, borderLeftWidth: 3, borderLeftColor: SAND, minHeight: 72 }}><Text style={{ lineHeight: 1.45 }}>{commentary?.trim() || 'Performance commentary was not added for this report.'}</Text></View>
