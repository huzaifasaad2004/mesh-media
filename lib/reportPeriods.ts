...
/** Inclusive [start, end] date range (YYYY-MM-DD) for a named period. end=null means open-ended (today/future). */
export function resolvePeriod(period: ReportPeriod, now = new Date()): { start: string | null; end: string | null } {
  ...
  switch (period) {
    ...
    case 'this_month':
      return { start: iso(new Date(y, m, 1)), end: null }
    case 'last_month':
      return { start: iso(new Date(y, m - 1, 1)), end: iso(new Date(y, m, 0)) }
    case 'this_quarter': {
      const qStartMonth = Math.floor(m / 3) * 3
      return { start: iso(new Date(y, qStartMonth, 1)), end: null }
    }
    case 'this_year':
      return { start: iso(new Date(y, 0, 1)), end: null }
    case 'all_time':
    default:
